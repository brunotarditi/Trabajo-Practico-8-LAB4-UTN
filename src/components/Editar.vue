  ...
  <div>
    ...
    >
      <b-form @submit="onSubmit">
        ...
        <b-button @click="hideModal" class="m-2" variant="danger"
          >Cerrar</b-button
        >
...
export default {
  ...
  methods: {
    getInstrumentoPorId(id) {
      axios.get("http://localhost:8080/instrumento/" + id).then((res) => {
        this.form = res.data;
        this.imagen = "http://localhost:8080/upload/files/" + this.form.imagen
      });
    },
    onSubmit(event) {
      ...
      axios
        .put("http://localhost:8080/instrumento/" + parametroId, this.form)
        .then((res) => {
          console.log(res.data);
          const upload = new FormData();
          upload.append("file", this.formData);
          axios.post("http://localhost:8080/upload", upload).then((res) => {
            console.log(res.data);
          });
          this.goProductos();
        });
